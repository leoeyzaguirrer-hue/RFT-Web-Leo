...
document.addEventListener("DOMContentLoaded", () => {
  // ==========================
  // 1. Animación de entrada (igual que index, pero aquí en JS)
  // ==========================
  setTimeout(() => {
  ...
  document.querySelectorAll(".ua-mini-item").forEach(item => {
    const correct = item.dataset.correct; // "yes" o "no"
    ...
    buttons.forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        if (ans === correct) {
          feedbackBox.textContent = "✔ Correcto: está descrito como episodio de acción-en-contexto.";
          feedbackBox.classList.add("ua-mini-ok");
          feedbackBox.classList.remove("ua-mini-error");
        } else {
          feedbackBox.textContent = "✖ No del todo: revisa si aparece contexto, acción concreta y consecuencias.";
